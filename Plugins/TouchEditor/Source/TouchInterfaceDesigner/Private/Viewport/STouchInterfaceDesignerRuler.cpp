...
struct STouchInterfaceDesignerRuler::FScrubRangeToScreen
{
...
struct STouchInterfaceDesignerRuler::FDrawTickArgs
{
...
void STouchInterfaceDesignerRuler::Construct(const STouchInterfaceDesignerRuler::FArguments& InArgs)
{
...
float STouchInterfaceDesignerRuler::DetermineOptimalSpacing(float InPixelsPerInput, uint32 MinTick, float MinTickSpacing) const
{
...
void STouchInterfaceDesignerRuler::SetRuling(FVector2D InAbsoluteOrigin, float InSlateToUnitScale)
{
	AbsoluteOrigin = InAbsoluteOrigin;
	SlateToUnitScale = InSlateToUnitScale;
}

void STouchInterfaceDesignerRuler::SetCursor(TOptional<FVector2D> InAbsoluteCursor)
{
	AbsoluteCursor = InAbsoluteCursor;
}

int32 STouchInterfaceDesignerRuler::DrawTicks(FSlateWindowElementList& OutDrawElements, const struct FScrubRangeToScreen& RangeToScreen, FDrawTickArgs& InArgs) const
{
...
int32 STouchInterfaceDesignerRuler::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
FReply STouchInterfaceDesignerRuler::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FReply STouchInterfaceDesignerRuler::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}

FReply STouchInterfaceDesignerRuler::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}

FVector2D STouchInterfaceDesignerRuler::ComputeDesiredSize(float LayoutScaleMultiplier) const
{
	return Orientation == Orient_Horizontal ? FVector2D(100, 18) : FVector2D(18, 100);
}

FReply STouchInterfaceDesignerRuler::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}