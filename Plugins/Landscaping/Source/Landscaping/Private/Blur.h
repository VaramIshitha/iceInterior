...
public:
    //!
    //! \fn void std_to_box(int boxes[], double sigma, int n)  
    //!
    ...
    //!
    static void std_to_box(int boxes[], double sigma, int n)  
    {
        // ideal filter width
        double wi = std::sqrt((12*sigma*sigma/n)+1); 
        int wl = std::floor(wi);  
        if(wl%2==0) wl--;
        int wu = wl+2;
    ...
    //!
    //! \fn void horizontal_blur(double * in, double * out, int w, int h, int r)    
    //!
    ...
    //!
    static void horizontal_blur(double * in, double * out, int w, int h, int r) 
    {
        ...
        for(int i=0; i<h; i++) 
        {
            ...
            double fv = in[ti], lv = in[ti+w-1], val = (r+1)*fv;
    ...
    //!
    //! \fn void total_blur(double * in, double * out, int w, int h, int r)   
    //!
    ...
    //! \param[in] r            box dimension
    //!
    static void total_blur(double * in, double * out, int w, int h, int r) 
    {
        double iarr = 1.0 / (r+r+1);
        #pragma omp parallel for
        for(int i=0; i<w; i++) 
        {
            ...
            double fv = in[ti], lv = in[ti+w*(h-1)], val = (r+1)*fv;
    ...
    //!
    //! \fn void box_blur(double * in, double * out, int w, int h, int r)    
    //!
    ...
    //! \param[in] r            box dimension
    //!
    static void box_blur(double *& in, double *& out, int w, int h, int r) 
    {
        std::swap(in, out);
    ...
    //!
    //! \fn void fast_gaussian_blur(double * in, double * out, int w, int h, double sigma)   
    //!
    ...
    //! \param[in] r            box dimension
    //!
    static void fast_gaussian_blur(double *& in, double *& out, int w, int h, double sigma) 
    {
