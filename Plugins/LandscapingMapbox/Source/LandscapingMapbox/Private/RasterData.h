...
struct RasterData
{
    ...
    FString ToString()
    {
        return FString::Printf(TEXT("Extents: %f,%f,%f,%f - %s - Import Res: %s - Landscape Res: %s - Meter Per Pixel: %s - Altitude: %f to %f - Location: %f,%f,%f - %s"), 
            Bottom, Left, Top, Right, *Projection, *ImportResolution.ToString(), *LandscapeResolution.ToString(), *MeterPerPixel.ToString(), MinAltitude, MaxAltitude, LocationX, LocationY, LocationZ, *Filename);
    }
};